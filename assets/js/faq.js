document.addEventListener('DOMContentLoaded', function() {
    ...
    const faqFromUrl = urlParams.get('faq');

    if (faqFromUrl) {
        const allInfos = document.querySelectorAll('.faq-full-info');
        allInfos.forEach(info => info.style.display = 'none');

        const targetInfo = document.querySelector(`.faq-full-info[data-faq="${faqFromUrl}"]`);
        if (targetInfo) {
